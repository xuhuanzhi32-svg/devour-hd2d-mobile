...
for(const n of ['progression','journey','narrative','game','navigation'])vm.runInContext(scripts.find(s=>s.startsWith('window.__HD2D_MODULES["'+n+'"]')),c);
...
function run(options){
 ...
 const s=g.state,record={...options,result:s.mode,seconds:+s.time.toFixed(2),zones:s.stats.zonesCleared,hp:+s.hero.hp.toFixed(2),propsBroken:s.stats.propsBroken||0,maxChain:s.stats.maxChain||0,perfectDodges:s.stats.perfectDodges||0,bossBreaks:s.stats.bossBreaks||0,npcTalks:s.stats.npcTalks||0,sideQuests:s.sideQuestLog?.length||0,storyBeats:s.storySeen?.length||0,attacks};
...
fs.mkdirSync(path.join(__dirname,'../qa'),{recursive:true});fs.writeFileSync(path.join(__dirname,'../qa/playthrough-v110.json'),JSON.stringify({kind:'public-api-accelerated-simulation',noStateInjection:true,notBrowserPlaythrough:true,results},null,2)+'\n');
